...
SignorSIF[,1] <- gsub(" ","_sp_",SignorSIF[,1],fixed = T)
SignorSIF[,3] <- gsub(" ","_sp_",SignorSIF[,3],fixed = T)
write.table(x = SignorSIF,file = "SignorSIF_NoHyphenNoSlashNoColonNoSpace.tsv",sep="\t",quote = F,row.names = F,col.names = F)
