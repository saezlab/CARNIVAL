...
![Figure 1: CARNIVAL  pipeline](./vignettes_pictures/carnival_Overview.png =100x100)
...
#colnames(toy_network_ex1) <- c("source","interaction", "target")
# lpSolve
#resultsLpSolve <- runVanillaCarnival( perturbations = toy_inputs_ex1, 
...
#result = runInverseCarnival(measObj = toy_measurements_ex2, netObj = toy_network_ex2)
```
which generates the following output:
```{r}
#print(result)
...
```
which writes two files, an Lp File and a parsed Data file. The next function takes  
```{r}

# resultsLpSolve <- runCarnivalFromLp(lpFile = "toy_files_vignettes/lpFile_t18_01_53d28_04_2021n4.lp",
