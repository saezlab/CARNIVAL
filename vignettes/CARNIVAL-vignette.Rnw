...
%\VignetteKeywords{Contextualizing large scale signalling networks from expression footprints with CARNIVAL }
...
\title{Contextualizing large scale signalling networks from expression footprints with \emph{CARNIVAL} \\\small{(version 1.0.0
and above)}}




\author[1,2]{Enio Gjerga}
...
<<eval=FALSE, pgf=FALSE, echo=TRUE>>=
...
\bibitem{OmniPath}
D.~Turei, T.~Korcsmaros, and J.~Saez-Rodriguez
