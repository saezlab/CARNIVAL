readOutResult <- function(cplexSolutionFileName = cplexSolutionFileName, variables = variables, conditionIDX = conditionIDX,
                              pknList = pknList, dir_name = dir_name, Export_all = Export_all, inputs=inputs, measurements=measurements,UP2GS=F){
  ...
  # up_xref <- UniProt.ws::select(up, keys = uniprot_names , columns = c("GENES"), keytype = "UNIPROTKB") #Map
  ...
  for(ii in 1:ncol(solMatrix)){
    ...
    if(length(kk1) > 0){
      ...
      for(i in 1:length(kk1)){
        ...
        if(as.numeric(edgesUp[kk1[i], 2])==as.numeric(nodesUp[which(nodesUp[, 1]==variables[[conditionIDX]]$variables[which(variables[[conditionIDX]]$exp==paste0("SpeciesUP ", tt, " in experiment ", conditionIDX))]), 2])){
          ...
          if (UP2GS) {
            ss <- IDmap[which(IDmap[,1]==ss),3]
            tt <- IDmap[which(IDmap[,1]==tt),3]
          }
    ...
    # map sif
    if (UP2GS) {
      for (counter_sif in 1:nrow(sif)) {
        sif[counter_sif,1] <- IDmap[which(IDmap[,1]==sif[counter_sif,1]),3]
        sif[counter_sif,3] <- IDmap[which(IDmap[,1]==sif[counter_sif,3]),3]
      }
    }
    ...
    # # map Dot_text
    # 
    # for (counter_dot in 3:length(Dot_text)) { # ignore the first two lines
    #   SourceTarget <- strsplit(substr(x = Dot_text[counter_dot],start = 1,stop = 14))
    # }
    ...
    if (length(idx)!=0) {
      ...
      if (UP2GS) {
        for (counter_actNode in 1:nrow(activityNodes)) {
          activityNodes[counter_actNode,1] <- IDmap[which(IDmap[,1]== activityNodes[counter_actNode,1]),3]
        }
      }
    ...
    if (!is.null(sif)) {
      ...
      if (UP2GS) {
        for (counter_input in 1:length(inputs)) {
          colnames(inputs)[counter_input] <- IDmap[which(IDmap[,1]==colnames(inputs)[counter_input]),3]
        }
      }
      ...
      if (UP2GS) {
        for (counter_meas in 1:length(measurements)) {
          colnames(measurements)[counter_meas] <- IDmap[which(IDmap[,1]==colnames(measurements)[counter_meas]),3]
        }
      }
  ...
  if(length(sifAll)==0){
    ...
    if (UP2GS) {
      for (counter_attr in 1:nrow(nodesAttributes)) {
        if (length(IDmap[which(IDmap[,1]==nodesAttributes[counter_attr,1]),3])>0) { # check if UniprotID could be mapped
          nodesAttributes[counter_attr,1] <- IDmap[which(IDmap[,1]==nodesAttributes[counter_attr,1]),3][1] # take only the first entry for multiple Gene Symbols
        }
      }
    }
