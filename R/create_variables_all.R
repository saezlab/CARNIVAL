...
create_variables_all_v_2_1 <- function(pknList = pknList, dataMatrix = dataMatrix){

  ## ======================================= ##
  ## ======= Load create_variables.R ======= ##
  ## ======================================= ##

  create_variables_v_2_1 <- function(priorKnowledgeNetwork = priorKnowledgeNetwork, 
    ...
    expNodes <- paste0("Species ", dataMatrix$species)
    expNodesReduced = paste0("Species ", dataMatrix$species)
    expNodesUp <- paste0("SpeciesUP ", dataMatrix$species)
    expNodesDown <- paste0("SpeciesDown ", dataMatrix$species)
    ...
    idxEdgesUp <- seq(from = length(nodesALL)+1, to = length(nodesALL)+
                        1+nrow(priorKnowledgeNetwork)*nrow(dataMatrix$dataMatrix)-1, by = 1)
    
    idxEdgesDown <- seq(from = length(nodesALL)+
                          1+nrow(priorKnowledgeNetwork)*nrow(dataMatrix$dataMatrix), 
                        to = length(nodesALL)+1+
                          nrow(priorKnowledgeNetwork)*nrow(dataMatrix$dataMatrix)+
                          nrow(priorKnowledgeNetwork)*nrow(dataMatrix$dataMatrix)-1, by = 1)
  ...
  dM <- dataMatrix
  dM$dataMatrix <- as.matrix(t(dataMatrix$dataMatrix[1, ]))
  dM$dataMatrixSign <- as.matrix(t(dataMatrix$dataMatrixSign[1, ]))
  
  res <- create_variables(priorKnowledgeNetwork = pknList, 
                                           dataMatrix = dM)
