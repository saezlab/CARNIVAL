...
create_variables_all_experiments <- function(pknList=pknList, dataMatrix=dataMatrix){
  ...
  create_variables <- function(pknList=pknList, dataMatrix = dataMatrix, conditionIDX=conditionIDX){
    ...
    cnt <- length(nodesALL)+length(edgesUp)+length(edgesDown)+1
    edges <- paste0("xb", cnt:(cnt + nrow(pknList)*nrow(dataMatrix$dataMatrix)-1), "_", conditionIDX)
