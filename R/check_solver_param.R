...
params <- list(
  timelimit =     c(is.numeric,
                    message="CPLEX parameter: Please set a time limit for CPLEX optimisation in
                             seconds"),

  mipGap =        c(is.numeric, 
                    message="CPLEX parameter: Please set the allowed
                              mipGAP parameter or leave it as NULL for CPLEX
                              default value (1e-04)"),

  poolrelGap =    c(is.numeric,
                    message="CPLEX parameter: Please set the allowed pool relative GAP parameter
                              or leave it as NULL for CPLEX default value (1e75)"),

  limitPop =      c(is.numeric,
                    message="CPLEX parameter: Please set the allowed population limit of solution
                             to be generated or leave it as NULL for CPLEX default value (20)"),

  poolCap =       c(is.numeric,
                    message="CPLEX parameter: Please set the allowed number of solutions to be
                              kept or leave it as NULL for CPLEX default value (2.1e9)"),

  poolIntensity = c(c(0:4),
                  message="CPLEX parameter: Please set the level of intensity for solution
                    searching [0,1,2,3,4] or leave it as NULL for CPLEX default value (0)
                    - to be decided by CPLEX"),

  poolReplace =   c(c(0:2),
                    message="CPLEX parameter: Please set the replacement strategy of solution
                              [0,1,2] or leave it as NULL for CPLEX default value (0) - First In
                              First Out"),

  threads =       c(is.numeric, ">=0",
                    message="CPLEX parameter: Please set the number of threads to 0 for automatic
                              detection or any value > 0 for a specific number of threads."),

  alphaWeight =   c(is.numeric,
                    message="Objective Function: Please set a weight for mismatch penalty (will be
                              applied only when the weight of measurement is not defined)"),

  betaWeight =    c(is.numeric,
                    message="Objective Function: Please set a weight for node penalty")
