...
write_objective_function_all_multi <- function(dataMatrix = dataMatrix, variables = variables, alphaWeight=alphaWeight, betaWeight=betaWeight,scores=scores, measWeights=measWeights, conditionIDX = conditionIDX, deltaWeight = 0.0001 ) {
  ...
  write_objective_function <- function(dataMatrix = dataMatrix, variables = variables, alphaWeight=alphaWeight, betaWeight=betaWeight, scores = scores, measWeights = measWeights, conditionIDX = conditionIDX ){
    ...
    if(is.null(scores)){
      ...
      ##
      objectiveFunctionUpVec <- paste0(" + ", deltaWeight, " ", variables$variables[variables$idxEdgesUp])
      objectiveFunctionUp <- paste(objectiveFunctionUpVec, collapse = "")
      
      objectiveFunctionDownVec <- paste0(" + ", deltaWeight, " ", variables$variables[variables$idxEdgesDown])
      objectiveFunctionDown <- paste(objectiveFunctionDownVec, collapse = "")
