...
enrichCARNIVAL<-function(Result_dir="Results_CARNIVAL",universeFile=NULL,datasource='kegg',topEnrichAll=40,pathwayfilter=T,pValSig=0.05){
  ...
  if (datasource!="allc2") {
    ...
    file.copy(from=system.file(paste0("source_enrichment/c2.cp.v6.2.symbols.gmt"),package="CARNIVAL"),to=getwd(),overwrite=TRUE) # retrieve network file
    test<-getGmt(paste0("source_enrichment/c2.cp.v6.2.symbols.gmt"),
                 collectionType=BroadCollection(category="c2"),
                 geneIdType=SymbolIdentifier())
    file.remove(paste0("source_enrichment/c2.cp.v6.2.symbols.gmt"))
  }
