...
enrichCARNIVAL<-function(Result_dir="Results_CARNIVAL",universeFile=NULL,datasource='kegg',topEnrichAll=40,pathwayfilter=T,pValSig=0.05){
  ...
  if (datasource!="allc2") {
    file.copy(from=system.file(paste0("c2.cp.",datasource,".v6.2.symbols.gmt"),package="CARNIVAL"),to=getwd(),overwrite=TRUE) # retrieve network file
    test<-getGmt(paste0("c2.cp.",datasource,".v6.2.symbols.gmt"),
                 collectionType=BroadCollection(category="c2"),
                 geneIdType=SymbolIdentifier())
    file.remove(paste0("c2.cp.",datasource,".v6.2.symbols.gmt"))
  } else {
    file.copy(from=system.file(paste0("c2.cp.v6.2.symbols.gmt"),package="CARNIVAL"),to=getwd(),overwrite=TRUE) # retrieve network file
    test<-getGmt(paste0("c2.cp.v6.2.symbols.gmt"),
                 collectionType=BroadCollection(category="c2"),
                 geneIdType=SymbolIdentifier())
    file.remove(paste0("c2.cp.v6.2.symbols.gmt"))
  }
  
  map<-read.table(universeFile, header = T,stringsAsFactors = F)
  universe<-map$genesymbol
  ...
  ggsave(filename = paste0(Result_dir,"/CARNIVAL_enrichment_PW_All_",datasource,".pdf"),plot = g_bar, width=10, height=10)
    ...
    warning("No enrichment result for all nodes was generated due to insufficient sample size that could pass p-value threshold at 0.5")
  }
  ...
  if (length(map$uniprot)>0) {
    ...
    setup<-set_init%>%left_join(map, by='uniprot')%>%
      filter(AvgAct>0)%>%
      filter(NodeType!='S')  #Comment this out
    genesup<-setup$gene
    
    setdn<-set_init%>%left_join(map, by='uniprot')%>%
      filter(AvgAct<0)%>%
      filter(NodeType!='S')  #Comment this out
    genesdn<-setdn$gene
  ...
  if (length(kOver_up@pvalues)>0 & length(kOver_dn@pvalues)>0) {
  ...
  print("Enrichment analysis completed: Please check enrichment results in the result folder")
    ...
    warning("No enrichment result for up/down nodes was generated due to insufficient sample size that could pass p-value threshold at 0.5")
  }
