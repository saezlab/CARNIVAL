...
source("resources/ebits_installation_script.R")
...
# === Plug in your raw microarray/GEX here ===
...
for (counter in 1:(nrow(TF_activities)/2)) {
  ...
  if (SelectedDrug!=4) {
    ...
    RowNames_ILP <- c(RowNames_ILP,substring(rownames(TF_activities)[((counter-1)*2)+1],20,nchar(rownames(TF_activities)[((counter-1)*2)+1])-9))
  }
}
